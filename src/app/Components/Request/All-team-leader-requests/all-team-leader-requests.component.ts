...
export class AllTeamLeaderRequestsComponent implements OnInit {
  ...
  CloseRequest() {
    ...
    this.reqDescriptionObj.requestStatusId=2;
  ...
  solvedRequest()
  {
    ...
    this.reqDescriptionObj.requestStatusId=5;
    this.requestDescservive.AddRequestDescription(this.reqDescriptionObj).subscribe(e=>{
    this.requestsService.GetRequestByRequestId(this.requestIDforSolvedRequest).subscribe(res=>{
    this.RequestApproved=res;
    this.RequestApproved.requestStatusId=5;
    this.RequestApproved.IsAssigned=true;
    this.RequestApproved.IsSolved=true;
    this.requestsService.updateRequest(this.requestIDforSolvedRequest,this.RequestApproved).subscribe(e => {
      console.log("soved request?",this.RequestApproved),
      this.ngOnInit();
    })
  })
  this.DialogueForSolved=false;
})  
