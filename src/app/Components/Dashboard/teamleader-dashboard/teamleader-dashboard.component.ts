...
@Component({
  ...
  templateUrl: './teamleader-dashboard.component.html',
  styleUrls: ['./teamleader-dashboard.component.css']
})
export class TeamleaderDashboardComponent implements OnInit {
  ...
  RequestWaitingApproveLength: any
  RequestApproveLength: any
  RequestDoneLength: any
  RequestRejectLength: any
  RequestSystemClosedLength: any
  ...
  ngOnInit(): void {
    ...
    this.RequestRejectLength=0
    this.RequestDoneLength=0
    this.RequestApproveLength=0
    this.RequestSystemClosedLength=0
    this.RequestWaitingApproveLength=0
    ...
    this.projectService.GetAllProjectsByEmployeeId(this.LoggedInUserId).subscribe(projects => {
      ...
      //   })
      //   this.projectService.CountApproveProjects(ele.id).subscribe(
      //     c=>{
      //     ele.RequestApproveLength=c
      //     })
      //     this.projectService.CountWaitingApproveProjects(ele.id).subscribe(
      //     c=>{
      //     ele.RequestWaitingApproveLength=c
      //    })
      //   this.projectService.CountDoneProjects(ele.id).subscribe(
      //    c=>{
      //   ele.RequestDoneLength=c
      //    })
      //   this.projectService.CountRejectProjects(ele.id).subscribe(
      //    c=>{
      //    ele.RequestRejectLength=c
      //   })
      // this.projectService.CountSystemClosedProjects(ele.id).subscribe(
      //  c=>{
      //    ele.RequestSystemClosedLength=c
      //    }) 
