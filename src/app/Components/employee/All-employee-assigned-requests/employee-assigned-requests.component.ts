...
export class EmployeeAssignedRequestsComponent implements OnInit {
  ...
  addMoreDescription() {
    ...
    // this.reqDescriptionObj.requestId=3
    this.reqDescriptionObj.requestStatusId=3
  ...
  openDialogueDoneRStatus(requestId)
  {
    this.RequestId=requestId;

    this.DialogueRequestIsDone=true;
   this.reqDescriptionObj.requestId = requestId
  }
 requestIsdone()
 {
  
   if (this.reqDescriptionObj.description!="")  
   {
    this.reqDescriptionObj.requestId = this.RequestId
    this.reqDescriptionObj.requestStatusId=4
    this.requestDescriptionService.AddRequestDescription(this.reqDescriptionObj).subscribe(e => {
      this.requestService.GetRequestByRequestId(this.RequestId).subscribe(res=>{
        this.RequestObj=res       
        this.RequestObj.requestStatusId=4;
        this.requestService.updateRequest(this.RequestId,this.RequestObj).subscribe(e => {
          console.log("request Done ?",this.RequestObj),
          this.ngOnInit();
          this.DialogueRequestIsDone=false;
        })
      })
    })
   }
  
   console.log("this.reqDescriptionObjss",this.reqDescriptionObj)
 }
  onReject() {
