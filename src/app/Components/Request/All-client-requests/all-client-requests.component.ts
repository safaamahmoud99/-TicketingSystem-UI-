...
export class AllClientRequestsComponent implements OnInit {
 ...
 // clientName:string
  ...
  RequestObj:request;
  reqcodee:string;
  dialogueApprovedRequest:boolean;
  constructor(private requestService: RequestService, private projectteamservice: ProjectTeamService,
    ...
    this.projectObj = {
      startDate: new Date, endtDate: new Date, RequestClosedLength: 0, RequestInProgressLength: 0, RequestOpenedLength: 0,RequestApproveLength:0,RequestDoneLength:0,RequestRejectLength:0,RequestSystemClosedLength:0,RequestWaitingApproveLength:0,
      IsDeleted: false,
    ...
    this.reqDescriptionObj = {
      ...
      description: '', id: 0, requestId: 0, userId: this.LoggedInUserString,requestStatusId:0
    }
    this.reqImage = {
   ...
   this.messageService.clear();
    ...
    && this.reqObj.assetId!=0 && this.reqObj.projectSiteAssetId!=0 && this.reqObj.projectSiteAssetId!=0
      ...
      this.requestService.inserRequest(this.reqObj).subscribe(e => {
        ...
        this.reqDescriptionObj.requestStatusId=1;      
...
//     });
// }
  OpenDialogueForRApproval(RequestId)
     {
      this.reqDescriptionObj.requestId=RequestId;
      this.dialogueApprovedRequest=true;
     }
  ConfirmSolvedRequest()
  {    
    this.reqDescriptionObj.requestStatusId=6
    this.requestDescriptionService.AddRequestDescription(this.reqDescriptionObj).subscribe(e=>{
      this.requestService.GetRequestByRequestId(this.reqDescriptionObj.requestId).subscribe(res=>{
        this.RequestObj=res       
        this.RequestObj.requestStatusId=6;
        this.requestService.updateRequest(this.reqDescriptionObj.requestId,this.RequestObj).subscribe(e => {
          console.log("soved request?",this.RequestObj),
          this.ngOnInit();
        })
      })
    })
    this.dialogueApprovedRequest=false;
  }
 requestRejected()
  {
     
    this.reqDescriptionObj.description="Request Rejected ";
    this.reqDescriptionObj.requestStatusId=7
    this.requestDescriptionService.AddRequestDescription(this.reqDescriptionObj).subscribe(e=>{
      this.requestService.GetRequestByRequestId(this.reqDescriptionObj.requestId).subscribe(res=>{
        this.RequestObj=res;
        this.RequestObj.IsSolved=false;
        this.RequestObj.IsAssigned=false;
        this.RequestObj.requestStatusId=7;
        this.requestService.updateRequest(this.reqDescriptionObj.requestId,this.RequestObj).subscribe(e => {
          console.log("soved request?",this.RequestObj),
